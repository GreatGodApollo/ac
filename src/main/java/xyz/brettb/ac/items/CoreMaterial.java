...
/**
 * <b>CoreMaterial</b> - Data Values/Pre-flattening<br>
...
public enum CoreMaterial {
    ...
    /**
     * <b>CoreMaterial Paradox (Duplication Check)</b>
     * <p>
     ...
     * @since 1.1.0
     *
    private static final ImmutableSet<String> LEGACY_VALUES = VALUES.stream().map(CoreMaterial::getLegacy)
    ...
    */
    ...
    CoreMaterial(int data, String... legacy) {
        this.data = (byte) data;
        this.legacy = legacy;
    }
    ...
    private static CoreMaterial requestOldCoreMaterial(@NonNull String name, byte data) {
        ...
        CoreMaterial material = NAME_CACHE.getIfPresent(holder);
        if (material != null) return material;

        for (CoreMaterial materials : VALUES) {
    ...
    /**
     * Checks if CoreMaterial enum contains a material with the given name.
     * <p>
     * You should use {@link #matchCoreMaterial(String)} instead if you're going
     * to get the CoreMaterial object after checking if it's available in the list
    ...
    public static Optional<CoreMaterial> matchCoreMaterial(@NonNull String name) {
        return matchCoreMaterial(name, (byte) -1);
    }

    /**
     * Parses the given material name as an CoreMaterial.
    ...
    public static Optional<CoreMaterial> matchCoreMaterial(@NonNull String name, byte data) {
        ...
        Optional<CoreMaterial> oldMatch = matchCoreMaterialWithData(name);
        ...
        return matchDefinedCoreMaterial(format(name), data);
    }
    ...
    private static Optional<CoreMaterial> matchCoreMaterialWithData(String name) {
        for (char separator : new char[]{',', ':'}) {
            ...
            return matchDefinedCoreMaterial(mat, data);
        }
    ...
    /**
     * Parses the given material as an CoreMaterial.
     *
    ...
    public static CoreMaterial matchCoreMaterial(@NonNull Material material) {
        ...
        return matchDefinedCoreMaterial(material.name(), (byte) -1)
                .orElseThrow(() -> new IllegalArgumentException("Unsupported Material: " + material));
    }

    /**
     * Parses the given item as an CoreMaterial using its material and data value (durability).
     *
    ...
    public static CoreMaterial matchCoreMaterial(@NonNull ItemStack item) {
        Objects.requireNonNull(item, "Cannot match null ItemStack");
        String material = item.getType().name();
        return matchDefinedCoreMaterial(material,
                isDamageable(material) ? (byte) 0 : (byte) item.getDurability())
                .orElseThrow(() -> new IllegalArgumentException("Unsupported Material: " + material));
    }

    /**
     * Parses the given material name and data value as an CoreMaterial.
     ...
     * @return an CoreMaterial (with the same data value if specified)
     * @see #matchCoreMaterial(String, byte)
    ...
    private static Optional<CoreMaterial> matchDefinedCoreMaterial(@NonNull String name, byte data) {
        ...
        if (data <= 0 && (ISFLAT || !duplicated)) {
            ...
            Optional<CoreMaterial> xMat = Enums.getIfPresent(CoreMaterial.class, name).transform(Optional::of).or(Optional.empty());
            if (xMat.isPresent()) return xMat;
        }

        // CoreMaterial Paradox (Duplication Check)
        ...
        if (!ISFLAT && duplicated) return Optional.ofNullable(requestDuplicatedCoreMaterial(name, data));
        return Optional.ofNullable(requestOldCoreMaterial(name, data));
    }
    ...
    public static boolean isDuplicated(@NonNull String name) {
        ...
        // Don't use matchCoreMaterial() since this method is being called from matchCoreMaterial() itself and will cause a StackOverflowError.
        for (Map.Entry<CoreMaterial, CoreMaterial> duplicated : DUPLICATED.entrySet())
            if (duplicated.getKey().name().equals(name) || duplicated.getKey().anyMatchLegacy(name)) return true;
        return false;
    }

    /**
     * Gets the CoreMaterial based on the material's ID (Magic Value) and data value.<br>
    ...
    /**
     * A solution for <b>CoreMaterial Paradox</b>.
    ...
    private static CoreMaterial requestDuplicatedCoreMaterial(@NonNull String name, byte data) {
        ...
        return mat.name().charAt(mat.name().length() - 1) == 'S' ? Enums.getIfPresent(CoreMaterial.class, name).orNull() : mat;
    }
     ...
     * <b>Example:</b>
     * <blockquote><pre>
     *     CoreMaterial material = {@link #matchCoreMaterial(ItemStack)};
    ...
    public static boolean isOneOf(@NonNull Material material, List<String> materials) {
        ...
        for (String comp : materials) {
            ...
            Optional<CoreMaterial> mat = matchCoreMaterial(comp);
    ...
    /**
     * Parses an item from this CoreMaterial.
    ...
    /**
     * Parses the material of this CoreMaterial.
     *
    ...
    /**
     * Parses the material of this CoreMaterial and accepts suggestions.
     *
